...
function validateEvent(candidate) {
  try {
    var value = eventSchema.validateSync(candidate, {
      abortEarly: false,
      strict: true
    });
    return {
...
var todoSchema = yup.object().shape({
  timestamp: yup.mixed(),
  summary: yup.string(),
  productId: yup.string(),
  method: yup.string(),
  ...
  lastModified: dateTimeSchema,
  calName: yup.string()
}).test('xor', "object should have due or duration", function (val) {
  var hasDue = !!val.due;
  var hasDuration = !!val.duration;
  return hasDue && !hasDuration || !hasDue && hasDuration || !hasDue && !hasDuration;
}).noUnknown();

function validateTodo(candidate) {
  try {
    var value = todoSchema.validateSync(candidate, {
      abortEarly: false,
      strict: true
    });
    return {
